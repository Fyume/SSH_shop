...
（在阿里云上面租了个服务器47.106.104.150）

47.106.104.150:8080/SSH_test/
...
**⑭**我傻了 直接用spring的aop环绕增强来做权限管理不就不用写这么多判断session user的代码咯。。。（好吧 最后还是用struts2的拦截器（也是aop就是了）实现了）

15.今天搞懂了前端的load方法，哇真赞，局部刷新。就相当于框架路由配置的效果（或许底层是实现就是一样的）这里附带一下：

overflow-y:auto; overflow-x:auto;

这两个属性可以让引入的页面若超出父div则显示滑动条

16.对于一早说fastjson不能选择属性转string的我错了，原来有个方法可以加入配置。好吧，但是我都写好方法了。关于近期优化前端的交互，使用load()方法局部刷新的问题，会导致el中关于外键的其他属性会报hibernate的关闭session。（这是da当然的了）所以就一时头大，关闭延迟加载啊什么的。后面仔细想了想，ajax（客户端）加载页面的时候早就关闭session了，怎么搞都是不行，就只能是将数据直接传到ajax处理（当然，最不用改el的方法当然是用fastjson转了存到sessoin（强无敌））
