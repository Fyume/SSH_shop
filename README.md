...
1.相当于在hibernate创建一个数据库，而mybatis则直接编写sql语句（当然 hibernate也可以直接编写sql语句（nativeSQL），不通过hibernate的缓存 这样也可以节省资源，不过这就破坏了hibernate本身的orm思想（不用管数据库的详细细节，直接操作POJO（由模版查询，输入简单的查询语句，直接获取关联对象）））
